...
public final class RedstoneryCommand {
    ...
    private static int seeCircuit(CommandContext<ServerCommandSource> ctx) {
        ...
        for (Circuit circuit : getCircuits(ctx.getSource().getServer())) {
                if (circuit.getName().equals(StringArgumentType.getString(ctx, "name"))) {
        ...
        if (currentCircuit.getDescriptions().isEmpty()) {
                ctx.getSource().sendFeedback(() -> Text.translatable("commands.redstonery.see.no_descriptions"), true);
        } else {
                for (String description : currentCircuit.getDescriptions()) {
                        ctx.getSource().sendFeedback(() -> Text.of("- " + description), true);
                }
        }
    ...
    private static int saveSelection(CommandContext<ServerCommandSource> ctx) {
        ...
        if (stack.getItem() == Redstonery.REDSTONE_SELECTOR) {
                ...
                if (stack.getNbt().contains("redstonery.pos1")) {
                        ...
                        pos1 = new BlockPos(nbtPos1[0], nbtPos1[1], nbtPos1[2]);
                }
                        
                if (stack.getNbt().contains("redstonery.pos2")) {
                        int[] nbtPos2 = stack.getNbt().getIntArray("redstonery.pos2");

                        pos2 = new BlockPos(nbtPos2[0], nbtPos2[1], nbtPos2[2]);
                }

                if (pos1 == null && pos2 == null) {
                        throw new CommandException(Text.translatable("commands.redstonery.error.selection.empty"));
                } else if (pos1 == null || pos2 == null) {
                        throw new CommandException(Text.translatable("commands.redstonery.error.selection.not_complete"));
                }
                ...
                for (int x = Math.min(pos1.getX(), pos2.getX()); x <= Math.max(pos1.getX(), pos2.getX()); x++) {
                        for (int y = Math.min(pos1.getY(), pos2.getY()); y <= Math.max(pos1.getY(), pos2.getY()); y++) {
                                for (int z = Math.min(pos1.getZ(), pos2.getZ()); z <= Math.max(pos1.getZ(), pos2.getZ()); z++) {
                                        ...
                                        BlockState state = ctx.getSource().getWorld().getBlockState(currentBlock);

                                        try {
                                                saveBlock(ctx, blocks, currentBlock, state);
                                        } catch (Exception e) {
                ...
                throw new CommandException(Text.translatable("commands.redstonery.selection.player_not_holding_selector"));
        }
    ...
    private static void saveBlock(CommandContext<ServerCommandSource> ctx, Set<CircuitBlock> blocks, BlockPos pos, BlockState state) {
        ...
        boolean open = getOpen(state);
        int page = getPage(state, pos, ctx.getSource().getWorld());
        boolean inverted = getInverted(state);

        CircuitBlock circuitBlock = new CircuitBlock(Registries.BLOCK.getId(state.getBlock()).toString(), direction, powered, locked, delay, subtract, open, page, inverted);

        ctx.getSource().sendFeedback(() -> Text.of(new Gson().toJson(circuitBlock)), true);
    ...
    private static Direction getDirection(BlockState state) {
        ...
        if (state.getBlock() instanceof DispenserBlock)
                direction = state.get(DispenserBlock.FACING);
    ...
    private static int getPower(BlockPos pos, BlockState state, ServerWorld world) {
        ...
        if (state.getBlock() instanceof ComparatorBlock) {
                List<Method> methods = Arrays.asList(ComparatorBlock.class.getDeclaredMethods());
    ...
    private static boolean getLocked(BlockState state) {
        if (state.getBlock() instanceof RepeaterBlock) {
    ...
    private static boolean getSubtract(BlockState state) {
        if (state.getBlock() instanceof ComparatorBlock) {
    ...
    private static int getPage(BlockState state, BlockPos pos, ServerWorld world) {
        if (state.getBlock() instanceof LecternBlock) {
                return ((LecternBlockEntity)world.getBlockEntity(pos)).getCurrentPage();
        }
